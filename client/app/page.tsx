...
export default function CalendarComponent() {
  ...
  const handleConfirmBooking = async () => {
    if (!selectedDate || !selectedTime || !userData.name || !userData.phone) return;
    ...
    try {
      ...
      alert("Error: No se pudo confirmar la reserva. Por favor, intente nuevamente.");
  ...
  return (
    <div className="min-h-screen bg-cover bg-center bg-fixed bg-no-repeat  sm:p-4 md:p-8 flex flex-col items-center justify-center" style={{
        ...
        <div className="fixed top-4 right-4 left-4 mx-auto max-w-sm bg-green-50/90 backdrop-blur-sm text-green-900 px-6 py-4 rounded-2xl shadow-xl border border-green-100 animate-fade-in z-50">
          <div className="flex items-center space-x-2">
            ...
            <div>
              ...
              <p className="text-sm text-green-800">Tu turno ha sido agendado exitosamente.</p>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="relative bg-white/95 backdrop-blur-sm rounded-[1.8rem] sm:rounded-[2.5rem] shadow-[inset_0_1px_1px_rgba(0,0,0,0.12)] p-4 sm:p-8 space-y-6 min-h-[600px] sm:min-h-[700px] overflow-y-auto">
          ...
          <div className="py-2">
            <DayPicker
              ...
              formatters={{ 
                formatWeekdayName: (weekday) => format(weekday, "EEEEEE", { locale: es }).charAt(0).toUpperCase() + format(weekday, "EEEEEE", { locale: es }).slice(1) 
              }}
            />
          </div>
          ...
          <h3 className="text-lg font-semibold text-gray-900 text-center">Horarios disponibles</h3>
          ...
          <button
          onClick={() => setCurrentPage(prev => Math.min(Math.floor((times.length - 1) / ITEMS_PER_PAGE), prev + 1))}
          ...
          <div className="pt-6 border-t border-gray-200/50 space-y-6">
            {selectedDate && selectedTime ? (
              <>
                <div className="bg-gray-50/80 backdrop-blur-sm p-5 rounded-2xl">
                  <p className="text-gray-600 text-sm text-center">
                    ...
                    <br />a las <span className="font-medium text-black text-lg">{selectedTime} hs</span>
                  </p>
                </div>
              ...
              <p className="text-gray-500 text-sm text-center py-4">
                Selecciona una fecha y hora para tu reserva
              </p>
            )}
            ...
            <button 
              ...
              className={`w-full py-5 px-6 rounded-2xl text-white font-medium transition-all duration-300 
                ${selectedDate && selectedTime && userData.name && userData.phone
                  ? 'bg-black hover:bg-gray-900 shadow-xl hover:shadow-2xl transform hover:-translate-y-1' 
                  : 'bg-gray-200 cursor-not-allowed'}`} 
              disabled={!selectedDate || !selectedTime || !userData.name || !userData.phone}
            >
              Confirmar Reserva
            </button>
          </div>
